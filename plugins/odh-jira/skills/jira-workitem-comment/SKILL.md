...
if [[ ! "$TICKET_KEY" =~ ^[A-Z]+-[0-9]+$ ]]; then
  echo "Error: Invalid ticket key format. Expected PROJECT-NUMBER (e.g., RHAI-1234)"
  exit 1
fi
...
```text
User: Add a comment to RHAI-1234 saying the fix is ready for review
Assistant: [Creates simple text comment]
Comment added to RHAI-1234
View: https://redhat.atlassian.net/browse/RHAI-1234
```

### Code Block Comment

```text
User: Comment on RHAI-5678 with this error message: [paste error]
Assistant: [Creates ADF comment with code block]
Comment added to RHAI-5678 with formatted code block
View: https://redhat.atlassian.net/browse/RHAI-5678
```

### Structured Update

```text
User: Add a status update to RHAI-910 with these action items:
- Fix merged to main
- QA testing in progress
- Deployment scheduled for Friday
Assistant: [Creates ADF comment with bullet list]
Comment added to RHAI-910
View: https://redhat.atlassian.net/browse/RHAI-910
```
