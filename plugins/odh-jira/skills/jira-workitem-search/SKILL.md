...
1. **Project keys**: Validate against pattern `[A-Z]+` (e.g., RHAI, PROJ).
...
3. **Ticket keys**: Validate against pattern `[A-Z]+-[0-9]+` (e.g., RHAI-1234).
...
```
Example: `project = RHAI AND status != Done ORDER BY priority DESC, updated DESC`

**Recent updates in a project (last N days):**
```jql
project = <PROJECT> AND updatedDate >= -<N>d ORDER BY updated DESC
```
Example: `project = RHAI AND updatedDate >= -7d ORDER BY updated DESC`

**Tickets by status:**
```jql
project = <PROJECT> AND status = "<STATUS>" ORDER BY updated DESC
```
Example: `project = RHAI AND status = "In Progress" ORDER BY updated DESC`

**Tickets by type (single):**
```jql
project = <PROJECT> AND type = <TYPE> ORDER BY updated DESC
```
Example: `project = RHAI AND type = Feature ORDER BY updated DESC`

**Tickets by type (multiple):**
```jql
project = <PROJECT> AND type IN (<TYPE1>, <TYPE2>, ...) ORDER BY updated DESC
```
Example: `project = RHAI AND type IN (Feature, Initiative) ORDER BY updated DESC`
...
```
Example: `project = RHAI AND component = "Wheel Package Index" ORDER BY priority DESC`
...
```
Example: `key IN (RHAI-1234, RHAI-5678, RHAI-910) ORDER BY updated DESC`

**Tips for extracting values:**
- If user says "RHAI backlog", use `project = RHAI`
...
if [[ ! "$PROJECT" =~ ^[A-Z]+$ ]]; then
  echo "Error: Invalid project key format. Expected uppercase letters only (e.g., RHAI, PROJ)"
  exit 1
fi
...
```
Found 15 tickets matching: project = RHAI AND status = "In Progress"

KEY          STATUS        ASSIGNEE      UPDATED     SUMMARY
─────────────────────────────────────────────────────────────────────
RHAI-1234   In Progress   code-samurai      2024-03-20  Fix duplicate CI runs
RHAI-5678   In Progress   super-picky-reviewer    2024-03-19  Add wheel signing support
RHAI-910    In Progress   Unassigned    2024-03-18  Update documentation
...
```
...
1. RHAI-1234: Fix duplicate CI pipeline runs
   Status: In Progress → Code Review
   Assignee: code-samurai
   Priority: High
   Updated: 2024-03-20
   https://redhat.atlassian.net/browse/RHAI-1234

2. RHAI-5678: Add wheel signing support
   Status: In Progress
   Assignee: super-picky-reviewer
   Priority: Medium
   Updated: 2024-03-19
   https://redhat.atlassian.net/browse/RHAI-5678

3. RHAI-910: Update documentation for new API
   Status: In Progress
   Assignee: Unassigned
   Priority: Low
   Updated: 2024-03-18
   https://redhat.atlassian.net/browse/RHAI-910
```
...
```
Ticket keys: RHAI-1234, RHAI-5678, RHAI-910
...
```jql
project = RHAI AND text ~ "wheel building" ORDER BY updated DESC
```

### Custom Fields

For RHAI's status-reporting fields:

```jql
project = RHAI AND "Color Status" = Red ORDER BY updated DESC
```
...
```text
User: What's in the RHAI backlog?
Assistant: [Searches with: project = RHAI AND status != Done]
Found 47 tickets in the RHAI backlog
...
```text
User: Export all RHAI bugs to CSV
...
```text
User: /jira-workitem-search project = RHAI AND status = "In Progress"
Assistant: [Shows 15 results]
User: Only show high priority ones
Assistant: [Refines to: project = RHAI AND status = "In Progress" AND priority = High]
